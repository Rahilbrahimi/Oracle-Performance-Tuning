create or replace PROCEDURE get_transaction_2 (
...
BEGIN
    ...
    END IF;
    IF LENGTH(p_from_timestamp) > 100 OR LENGTH(NVL(p_to_timestamp,'')) > 100 THEN
        RAISE_APPLICATION_ERROR(-20005, 'طول تاریخ نباید بیشتر از 100 کاراکتر باشد');
    END IF;
