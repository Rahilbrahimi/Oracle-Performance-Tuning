...
---sakht function json az data khata.
  PROCEDURE raise_error(
    p_error_code IN VARCHAR2
  );
---yek oracle exception partab mikone.
...
/
---khataro modiriyat mikone.json barmighardoone.
...
---FERESTADAN KHATA
  ...
  BEGIN
    ...
    IF p_error_code IS NULL THEN
      l_err_code := l_err_code || ' (' || NVL(SQLERRM, 'System error') || ')';
    END IF;
